...
export interface SupplementaryPassResult {
  additionalAssets: DiscoveredAsset[];
  inputTokens: number;
  outputTokens: number;
  costUsd: number;
}

export async function runSupplementaryPass(
  companyName: string,
  isin: string,
  sector: string,
...
): Promise<SupplementaryPassResult> {
  const assetSummary = existingAssets.map((a, i) =>
    `${i + 1}. ${a.facility_name} (${a.asset_type}) - ${a.city}, ${a.country} [${a.ownership_share ?? 100}% owned]`
  ).join("\n");
...
ASSETS ALREADY IDENTIFIED BY ANOTHER AI MODEL (${existingAssets.length} total):
Types breakdown: ${typeSummary}

${assetSummary}

This list was generated by a different AI model. Your job is to find assets that model MISSED. Think carefully about:
- Countries where ${companyName} operates that may not be fully represented
- Joint ventures and partnerships with correct ownership percentages
- Recently acquired or under-construction facilities
- Infrastructure assets (ports, railways, pipelines, conveyor systems)
- Energy generation assets (power plants, renewable energy)
- Storage and logistics facilities
- R&D and technology centers in key markets
- Smaller regional offices or facilities in emerging markets

Only return NEW assets not already in the provided list. Do not duplicate any assets.`;

  console.log(`[Discovery v2] Starting supplementary pass for ${companyName} with provider ${supplementaryProviderId}`);
  const response = await callLLM(supplementaryProviderId, SUPPLEMENTARY_PROMPT, userPrompt, apiKeyOverride);

  if (!response.content) {
    return { additionalAssets: [], inputTokens: response.inputTokens, outputTokens: response.outputTokens, costUsd: response.costUsd };
  }

  let supplementary: any;
  try {
    supplementary = JSON.parse(response.content);
  } catch {
    console.warn(`[Discovery v2] Supplementary pass JSON parse failed for ${companyName}, attempting repair...`);
    try {
      supplementary = JSON.parse(repairJSON(response.content));
    } catch {
      console.warn(`[Discovery v2] Supplementary pass JSON repair also failed for ${companyName}, skipping`);
      return { additionalAssets: [], inputTokens: response.inputTokens, outputTokens: response.outputTokens, costUsd: response.costUsd };
    }
  }
